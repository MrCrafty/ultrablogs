...
const page = async () => {
    ...
    return (
        <div className='w-11/12 lg:w-1/2 mx-auto'>
            {count >= 1 ?
                <BlogList data={data} />
                : <p className='text-xl'>{pageData?.my_blogs?.error_message}<Link href={pageData?.my_blogs?.error_link} className='text-blue-400'> {pageData?.my_blogs?.error_text}</Link></p>}
        </div>
    )
}
